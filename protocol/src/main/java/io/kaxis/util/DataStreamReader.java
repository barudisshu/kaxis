...
/**
 * This class describes the functionality to read raw network-ordered data streams on bit-level.
 *
 * <p>For a {@link InputStream} the specification of the returned value of {@link
 * InputStream#available()} is weak, therefore the methods in this class are not based on that. If
 * the data is already read into a byte-array, {@link DatagramReader} may be used, in order to use
...
public class DataStreamReader {
  ...
  protected static class RangeInputStream extends ByteArrayInputStream {
    ...
    /**
     * Create reader for range. Read range and pass it to the returned reader.
     *
    ...
    private RangeInputStream range(int count) {
      int offset = pos;
      long available = super.skip(count);
      if (available < count) {
   ...
   * @throws IllegalArgumentException if provided numBits exceeds available bytes, or that value is
   *     out of the range {@code [0...64]}.
   */
   ...
   * @param full read always full length, may cause multiple reads on input stream.
   ...
   *
   * <p>Read first the length of the variable bytes according the size in {@code numBits}. For the
   * length {@code -1}, {@code null} is returned, and for the length {@code 0}, {@link
   * Bytes#EMPTY_BYTES} is returned.
   *
  ...
  public ByteArrayInputStream createRangeInputStream(int count) {
    ...
    if (byteStream instanceof RangeInputStream range) {
