 ...
 *
 * <p>Since 3.9 the used cipher definition (algorithm + key size) is contained in the encrypted
 * stream.
 *
 * <p>Format:
 *
...
public class EncryptedStreamUtil {
   ...
   * Default cipher.
   *
   * <p>Since 3.9, the default cipher is "AES/GCM/NoPadding", if supported by the JCE, otherwise
   * "AES/CBC/PKCS5Padding" (mandatory supported by Java 7).
   *
   * @deprecated use {@link #setDefaultWriteCipher()}
   */
  @Deprecated public static final String DEFAULT_CIPHER_ALGORITHM = "AES/CBC/PKCS5Padding";
  ...
  private static final class CipherDefinition {
    ...
    public boolean equals(Object obj) {
      ...
      if (keySizeBits != other.keySizeBits) return false;
      if (algorithm == null) {
        return other.algorithm == null;
      } else return algorithm.equals(other.algorithm);
    }
  }
   ...
   *
   * <p>"AES/GCM/128", if supported by the JCE, or "AES/CBC/128", if "AES/GCM/128" is not supported.
   *
   * @since 3.9
   */
   ...
   * @return {@code true}, if cipher was added, {@code false}, if cipher is not supported
   * @since 3.9
   */
   ...
   * Add cipher.
   *
   * <p>Adding ciphers is limited. Ciphers may require specific parameters and fail for that reason.
   *
   * @param id persistent id. Custom id must be in range {@code 0xf000-0xfeff}.
   * @param algorithm cipher algorithm
   * @param bits key size in bits
   * @since 3.9
   * @return {@code true}, if cipher is supported, {@code false}, if cipher is not supported
   * @throws IllegalArgumentException if id or algorithm and bits are already added or the id is not
   *     in custom range.
   * @since 3.9
   */
      ...
      if (definitionId.equals(definitionParam)) {
        ...
        throw new IllegalArgumentException(
            "0x"
                ...
                + " already in use for "
                + definitionId.algorithm
                + "/"
                + definitionId.keySizeBits
                + "!");
      }
    } else if (definitionParam != null) {
      throw new IllegalArgumentException(
          definitionParam.algorithm
              + "/"
              + definitionParam.keySizeBits
              + " already defined as 0x"
              + Integer.toHexString(id)
              + "!");
    }
  ...
  /**
   * Create encrypted serialization utility with provided algorithm and key size.
   *
   * @param cipherAlgorithm cipher algorithm
   * @param keySizeBits key size in bits
   ...
   *
   * @return cipher specification (algorithm + key size). e.g. "AES/GCM/128". {@code null}, if not
   *     available
   * @since 3.9
   */
  ...
  public void setWriteCipher(String cipherAlgorithm, int keySizeBits) {
    ...
    if (definition == null || !definition.supported) {
      throw new IllegalArgumentException(
          cipherAlgorithm + "/" + keySizeBits + " is not supported!");
    }
   ...
   * Initialize cipher.
   *
   * @param mode mode for {@link Cipher#init(int, java.security.Key, AlgorithmParameterSpec)}.
   *     {@link Cipher#DECRYPT_MODE} or {@link Cipher#ENCRYPT_MODE}
   * @param password password
   * @param seed seed. Either randomly generated when saving, or read from persistence, when
   *     loading.
   ...
   *
   * <p>Since 3.9 reads also cipher definition.
   *
   ...
   * @param password password for decryption. If {@code null}, the input stream must be not
   *     encrypted, but starts with an empty seed (byte 0).
   * @return prepared input stream. If a "seed" is found at the head and password is provided, a
   *     {@link CipherInputStream}. If that doesn't {@link InputStream#markSupported()}, wrapped
   *     with a {@link BufferedInputStream}. If a "seed" is found, but the password is missing or
   *     the cipher algorithm isn't supported, a empty input stream is returned and no items are
   *     loaded.
   */
   ...
   * Prepare input stream.
   *
   * <p>Provided seed must be read using {@link #readSeed(InputStream)} before.
   *
   * @param seed seed to decrypt data. If {@code null}, don't decrypt.
   * @param in input stream to read data from
   * @param password password for decryption. If {@code null}, the input stream must be not
   *     encrypted, and the seed must be [@link null} or empty.
   * @return prepared input stream. If a seed is provided and password is provided, a {@link
   *     CipherInputStream}. If that doesn't {@link InputStream#markSupported()}, wrapped with a
   *     {@link BufferedInputStream}. If a seed is provided, but the password is missing or the
   *     cipher algorithm isn't supported, a empty input stream is returned and no items are loaded.
  ...
  public InputStream prepare(byte[] seed, InputStream in, SecretKey password) {
    if (seed != null && seed.length > 0) {
      if (password == null) {
        ...
        return new ByteArrayInputStream(Bytes.EMPTY_BYTES);
      }
   ...
   *
   * <p>Writes random seed, if password is provided, or {@link Bytes#EMPTY_BYTES}, if no password is
   * provided.
   *
   * @param out output stream to write data to
   * @param password password for encryption. If {@code null}, the output stream is not encrypted,
   *     but starts with an empty seed (byte 0).
   * @return prepared output stream, {@link CipherOutputStream}, if a password is provided.
   * @throws IOException if an i/o-error occurred or no new random seed could be generated.
   */
   ...
   * provided.
   *
   * @param seed seed the current/last written/read data is using. Ensure, a new seed is used to
   *     write. {@code null}, if not available.
   * @param out output stream to write data to
   * @param password password for encryption. If {@code null}, the output stream is not encrypted,
   *     but starts with an empty seed (byte 0).
   * @return prepared output stream, {@link CipherOutputStream}, if a password is provided.
   ...
   * @since 3.8
   */
  public OutputStream prepare(byte[] seed, OutputStream out, SecretKey password)
      throws IOException {
    ...
    if (password == null) {
      writer.writeVarBytes(Bytes.EMPTY_BYTES, Byte.SIZE);
      writer.writeTo(out);
    }
    return out;
  }
   ...
   *
   * <p>Note: This function is copied from Scandium / PseudoRandomFunction, otherwise this would
   * either create a dependency or a critical crypto function must be moved outside Scandium.
   *
