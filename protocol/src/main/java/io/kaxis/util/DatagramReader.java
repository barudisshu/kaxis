 ...
 * InputStream#available()} is weak. Therefore the methods in the base-class {@link
 * DataStreamReader} are not based on that. If the data is already read into a byte-array, this
 * class may be used in order to used methods, which are based on {@link
...
public final class DatagramReader extends DataStreamReader {
   ...
   *
   * @param byteArray The byte array to read from.
   * @param copy {@code true} to copy the array, {@code false} to us it directly.
   */
   ...
   *
   * <p>The array is used directly and is not copied. If a copy is required, copy the range and
   * provide that to {@link #DatagramReader(byte[])}.
   *
   * @param byteArray The byte array to read from.
   ...
   * {@inheritDoc}
   *
   * @param count The number of bytes to read. If value is negative, read left bytes.
   */
  @Override
   ...
   *
   * @param message message to include in {@link IllegalArgumentException} message.
   ...
   *
   * @return {@code true}, if there are bytes left to read, {@code false}, otherwise.
   */
   ...
   *     <em>expectedBytes</em>. {@code false}, otherwise.
   */
