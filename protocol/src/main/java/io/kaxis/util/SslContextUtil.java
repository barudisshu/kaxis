 ...
 *
 * <p>This utility converts between the informations used by the javax SSL implementation and the
 * plain credentials used by scandium. It offers reading KeyStores, extracting credentials, and
 * creating KeyManager and TrustManager from the KeyStores or extracted credentials. Java SSLContext
 * is able to maintain different key-certificate pairs for different key and signing algorithms. The
 * californium-scandium concentrates on embedded client and therefore supports only one key-signing
 * algorithm. This utility therefore helps, to select the right credentials or create a javax
 * security classes for javax SSL implementation from that selected credentials.
 *
 * <p>The utility provides a configurable mapping of URI endings with key store type, enable to use
 * different KeyStore implementations and formats. The URI ending is defined as the part starting
 * with the last "." after all "/" separator. e.g.: "cert/keyStore.p12" has ending ".p12", and
 * "cert/keyStore.p12/test" has no ending. The currently pre-configured mapping contains:
 *
 * <pre>
 * ".jks" to "JKS"
 * ".bks" to "BKS"
 ...
 * </pre>
 *
 * CRT/PEM Custom Reader: Read private keys (PKCS8 and PKCS12 (EC only)), public key (x509), and
 * certificates (x509). These credentials are stored in the PEM format, which is base64 encoded. The
 * sections of the file contains a description of the encoded credential. If a general load function
 * is used, {@link Credentials} are returned with the loaded data filled in.
 *
 * <p>Example:
 *
 * <pre>
 ...
 * </pre>
 *
 * You may use <a href="https://lapo.it/asn1js" target="_blank">lapo.it/asn1js</a> to decode the
 * content. Please ensure, that you only provide test- or demo-credentials.
 *
 * <p>If that example file is loaded with {@link #loadCredentials(String)} the returned {@link
 * Credentials} contains a private key, and here, also the corresponding public key.
 *
 * <p>The utility provides also a configurable input stream factory of URI schemes. Currently only
 * {@link #CLASSPATH_SCHEME} is pre-configured to load key stores from the classpath. If the scheme
 * of the URI has no configured input stream factory, the URI is loaded with {@link
 * URL#URL(String)}.
 *
 * <p>Note: It is not intended, that the configuration is changed during usage, this may cause race
 * conditions! Currently this class provides a class level access based API. Therefore only one
 * configuration at all is possible. Depending on the usage, this may change to instance level
 * access based API to support more parallel configurations to be used.
 *
...
public class SslContextUtil {
   ...
   */
  @Deprecated public static final Logger LOGGER = LoggerFactory.getLogger(SslContextUtil.class);
  ...
  /**
   * Default protocol used for {@link #createSSLContext(String, PrivateKey, X509Certificate[],
   * Certificate[])}.
   */
   ...
   *
   * @param trust trust definition keystore#hexstorepwd#aliaspattern or keystore.pem. If no
   *     aliaspattern should be used, just leave it blank keystore#hexstorepwd#
   ...
   * @throws IllegalArgumentException if trust doesn't match keystore#hexstorepwd#aliaspattern or no
   *     matching trusts are found
   * @throws NullPointerException if trust is {@code null}.
   * @see #PARAMETER_SEPARATOR
   */
  public static Certificate[] loadTrustedCertificates(String trust)
    ...
    if (3 != parameters.length) {
      throw new IllegalArgumentException(
          "trust must comply the pattern <keystore"
              + PARAMETER_SEPARATOR
              + "hexstorepwd"
              + PARAMETER_SEPARATOR
              + "aliaspattern>");
    }
    return loadTrustedCertificates(
        parameters[0], parameters[2], Utility.hex2CharArray(parameters[1]));
  }

  /**
   * Load credentials from key store.
   *
   * @param credentials credentials definition keystore#hexstorepwd#hexkeypwd#alias or keystore.pem
   * @return credentials
   * @throws IOException if key store could not be loaded.
   * @throws GeneralSecurityException if security setup failed.
   * @throws IllegalArgumentException if keys doesn't match keystore#hexstorepwd#hexkeypwd#alias or
   *     no matching trusts are found
   * @throws IncompleteCredentialsException if either private key or certificate chain and public
   *     key is missing.
   * @throws NullPointerException if credentials is {@code null}.
   * @see #PARAMETER_SEPARATOR
   * @since 3.0 IncompleteCredentialsException added
   */
  public static Credentials loadCredentials(String credentials)
      throws IOException, GeneralSecurityException {
    if (null == credentials) {
    ...
    if (4 != parameters.length) {
      throw new IllegalArgumentException(
          "credentials must comply the pattern <keystore"
              + PARAMETER_SEPARATOR
              + "hexstorepwd"
              + PARAMETER_SEPARATOR
              + "hexkeypwd"
    ...
    return loadCredentials(
        parameters[0],
        parameters[3],
        Utility.hex2CharArray(parameters[1]),
        Utility.hex2CharArray(parameters[2]));
  }

  /**
   * Load TrustManager from key store.
   *
   * @param keyStoreUri key store URI. Supports configurable URI scheme based input streams and URI
   *     ending based key store type.
   * @param aliasPattern regular expression for aliases to load only specific certificates for the
   *     TrustManager. null to load all certificates.
   ...
   */
  public static TrustManager[] loadTrustManager(
      String keyStoreUri, String aliasPattern, char[] storePassword)
      throws IOException, GeneralSecurityException {
    Certificate[] trustedCertificates =
        loadTrustedCertificates(keyStoreUri, aliasPattern, storePassword);
   ...
   * @param keyStoreUri key store URI. Supports configurable URI scheme based input streams and URI
   *     ending based key store type.
   * @param aliasPattern alias pattern to load only specific credentials into the KeyManager. null
   *     to load all credentials into the KeyManager.
   * @param storePassword password for key store.
   ...
   * @throws NullPointerException if keyStoreUri, storePassword, or keyPassword is {@code null}.
   * @since 3.0 (add support for alias pattern)
   */
  public static KeyManager[] loadKeyManager(
      String keyStoreUri, String aliasPattern, char[] storePassword, char[] keyPassword)
      throws IOException, GeneralSecurityException {
   ...
   * @param keyStoreUri key store URI. Supports configurable URI scheme based input streams and URI
   *     ending based key store type.
   * @param aliasPattern regular expression for aliases to load only specific certificates for
   *     trusting. null to load all certificates.
   * @param storePassword password for key store.
   * @return array with trusted certificates.
   * @throws IOException if key store could not be loaded.
   * @throws GeneralSecurityException if security setup failed.
  ...
  public static Certificate[] loadTrustedCertificates(
    ...
    if (trustedCertificates.isEmpty()) {
      throw new IllegalArgumentException(
          "no trusted x509 certificates found in '"
              + keyStoreUri
              + "' for '"
              + aliasPattern
              + "'!");
    }
    return trustedCertificates.toArray(new Certificate[0]);
  }
   ...
   * @throws IOException if key store could not be loaded.
   * @throws GeneralSecurityException if security setup failed.
   * @throws IllegalArgumentException if alias is empty, or no matching credentials are found.
   * @throws IncompleteCredentialsException if either private key or certificate chain and public
   *     key is missing.
   * @throws NullPointerException if keyStoreUri, storePassword, keyPassword, or alias is {@code
   *     null}.
   * @since 3.0 IncompleteCredentialsException added
   */
  public static Credentials loadCredentials(
      String keyStoreUri, String alias, char[] storePassword, char[] keyPassword)
      throws IOException, GeneralSecurityException {
    KeyStoreType configuration = getKeyStoreTypeFromUri(keyStoreUri);
    if (configuration.simpleStore != null) {
      Credentials credentials = loadSimpleKeyStore(keyStoreUri, configuration);
      if (credentials.publicKey == null && credentials.privateKey == null) {
        ...
        throw new IncompleteCredentialsException(
            credentials, "credentials missing! No private key found!");
      } else if (credentials.publicKey == null) {
        throw new IncompleteCredentialsException(
            credentials, "credentials missing! Neither certificate chain nor public key found!");
      }
      return credentials;
    }
    if (null == alias) {
    ...
    throw new IllegalArgumentException(
        "no credentials found for '" + alias + "' in '" + keyStoreUri + "'!");
  }
  ...
  public static PrivateKey loadPrivateKey(
    ...
    throw new IllegalArgumentException(
        "no private key found for '" + alias + "' in '" + keyStoreUri + "'!");
  }
  ...
  public static PublicKey loadPublicKey(String keyStoreUri, String alias, char[] storePassword)
    ...
    throw new IllegalArgumentException(
        "no public key found for '" + alias + "' in '" + keyStoreUri + "'!");
  }
   ...
   * @throws IOException if key store could not be loaded.
   * @throws GeneralSecurityException if security setup failed.
   * @throws IllegalArgumentException if alias is empty, or no matching certificate chain is found.
   * @throws NullPointerException if keyStoreUri, storePassword, or alias is {@code null}.
   */
  public static X509Certificate[] loadCertificateChain(
      String keyStoreUri, String alias, char[] storePassword)
      throws IOException, GeneralSecurityException {
    KeyStoreType configuration = getKeyStoreTypeFromUri(keyStoreUri);
    if (configuration.simpleStore != null) {
      Credentials credentials = loadSimpleKeyStore(keyStoreUri, configuration);
   ...
   *
   * <p>Clear previous configuration. Custom entry must be added again using {@link
   * #configure(String, InputStreamFactory)}, {@link #configure(String, KeyStoreType)}, and {@link
   * #configureAlias(String, String)}.
   */
    ...
    KeyStoreType simple =
        new KeyStoreType(
            new SimpleKeyStore() {
              ...
              @Override
              public Credentials load(InputStream inputStream)
   ...
   *
   * @param ending URI ending. If {@link #DEFAULT_ENDING} is used, the key store default type is
   *     configured. Ending is converted to lower case before added to the {@link #KEY_STORE_TYPES}.
   * @param type the key store type.
   * @return previous key store type, or {@code null}, if no key store type was configured before
   * @throws NullPointerException if ending or type is {@code null}.
   * @throws IllegalArgumentException if ending doesn't start with "." and isn't {@link
   *     #DEFAULT_ENDING}.
   ...
   *
   * <p>Use the same {@link KeyStoreType} for the alias as for the ending.
   *
   * @param alias new alias
   * @param ending already configured ending
   * @return previous key store type, or {@code null}, if no key store type was configured before
   * @throws NullPointerException if alias of ending is {@code null}.
   * @throws IllegalArgumentException if alias and ending are equal, or alias or ending doesn't
   *     start with "." and isn't {@link #DEFAULT_ENDING}.
   * @since 3.6
   */
   ...
   *
   * @param scheme URI scheme. Scheme is converted to lower case before added to the {@link
   *     #INPUT_STREAM_FACTORIES}.
   * @param streamFactory input stream factory to read key stores access with this URI scheme.
   * @return previous stream factory, if already configure, or {@code null}, if not stream factory
   ...
   *
   * <p>Get the configured key store type for URI ending from {@link #KEY_STORE_TYPES}. If no key
   * store type for URI ending is available, get the key store type for {@link #DEFAULT_ENDING}.
   *
   * @param uri URI provide ending for lookup. Converted to lower case before used.
   * @return configured key store type for ending or default, if ending is not configured.
   ...
   * Get scheme from URI.
   *
   * <p>Use {@link #SCHEME_DELIMITER} to split scheme from URI.
   *
   ...
   *
   * <p>Create input stream from URI. If no scheme is provided ({@link #SCHEME_DELIMITER} not
   * found), open a file using the URI. Otherwise, if a scheme is provided, use that scheme to
   * lookup a configured {@link InputStreamFactory}. If no factory for that scheme was configured
   * with {@link #configure(String, InputStreamFactory)}, then use {@link URL}.
   *
   * @param keyStoreUri URI of input stream
   * @return input stream
    ...
    if (scheme == null) {
      ...
      if (errorMessage == null) {
        ...
        throw new IOException(
            "URI: " + keyStoreUri + ", file: " + file.getAbsolutePath() + errorMessage);
      }
    } else {
   ...
   * Load key store.
   *
   * @param keyStoreUri key store URI. Use {@link #getInputStreamFromUri(String)} to read the key
   *     store, and {@link #getKeyStoreTypeFromUri(String)} to determine the type of the key store.
   * @param storePassword password for key store.
  ...
  private static KeyStore loadKeyStore(
      String keyStoreUri, char[] storePassword, KeyStoreType configuration)
      throws GeneralSecurityException, IOException {
   ...
   *
   * @param keyStoreUri key store URI. Use {@link #getInputStreamFromUri(String)} to read the simple
   *     key store
   * @param configuration the key store configuration to read the simnple key store
   * @return credentials
   ...
   *
   * <p>Ensure, that all provided certificates are x509.
   *
   * @param certificates to check
   * @return array with x509 certificates.
   * @throws IllegalArgumentException if null, a empty array is provided or a none x509 certificate
   *     was found or a array entry was null.
   */
    ...
    for (int index = 0; certificates.length > index; ++index) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "["
                + index
                + "] is not a x509 certificate! Instead it's a "
   ...
   * @throws IllegalArgumentException if key managers are empty, or no x509 key manager is available
   * @since 3.0
   */
  ...
  public static void ensureUniqueCertificates(X509Certificate[] certificates) {
    ...
    for (X509Certificate certificate : certificates) {
      if (!set.add(certificate)) {
        throw new IllegalArgumentException(
            "Truststore contains certificates duplicates with subject: "
   ...
   *
   * <p>Uses {@link #DEFAULT_SSL_PROTOCOL}.
   *
   * @param alias alias to be used in KeyManager. Used for identification according the
   *     X509ExtendedKeyManager API to select the credentials matching the provided key. Though the
   *     create KeyManager currently only supports on set of credentials, the alias is only used to
   *     select that. If {@code null}, it's replaced by a default "californium".
   ...
   * @throws GeneralSecurityException if security setup failed.
   * @throws NullPointerException if private key, or the chain, or the trusts is {@code null}.
   * @throws IllegalArgumentException if the chain or trusts is empty.
   */
  public static SSLContext createSSLContext(
      String alias, PrivateKey privateKey, X509Certificate[] chain, Certificate[] trusts)
   ...
   *
   * <p>Intended to be used, if {@link JceProvider#hasStrongEncryption()} returns {@code false}.
   * Selects the "AES_128" cipher suites only.
   *
   * @param sslContext context to get list of weak cipher suites.
   * @return array with weak cipher suites, or {@code null}, if all cipher suites of the context are
   *     already weak or no weak one is available in the context at all.
   * @since 3.0
   */
  ...
  /**
   * Create key manager from private key and certificate path. Creates a {@link KeyStore} to use
  ...
  public static KeyManager[] createKeyManager(
      String alias, PrivateKey privateKey, X509Certificate[] chain)
      throws GeneralSecurityException {
    if (null == privateKey) {
  ...
  /**
   * Create trust manager from trusted certificates. Creates a {@link KeyStore} to use {@link
   * TrustManagerFactory}.
   *
   * @param alias alias to be used for key store.
   * @param trusts trusted certificates
   ...
   * @throws GeneralSecurityException if security setup failed.
   */
  private static KeyManager[] createKeyManager(KeyStore store, char[] keyPassword)
      throws GeneralSecurityException {
   ...
   * Credentials.
   *
   * <p>Pair of private key and public key or certificate chain. Or set of trusted certificates.
   */
  public static class Credentials implements Destroyable {

    /**
     * Indicates, that this isntance has been {@link #destroyed}ed.
     */
     ...
     * @param chain certificate chain
     * @throws IllegalArgumentException if public key and chain is provided, but the public key
     *     doesn't match the one of the certificates head
     */
     ...
     * Create credentials.
     *
     * @param trusts certificate trusts, {@code null} for no trusted certificates.
     */
     ...
     *
     * @return certificate trust chain. May be {@code null}, if not available.
     */
     ...
     *
     * @return certificate trust chain as list. May be {@code null}, if not available.
     * @since 3.0
     */
   ...
   *
   * <p>Missing private or public key.
   *
   * @since 3.0
   */
  public static class IncompleteCredentialsException extends IllegalArgumentException {
     ...
     *
     * <p>Either private or public key is missing.
     */
    ...
    public IncompleteCredentialsException(
        Credentials incompleteCredentials, String message, Throwable cause) {
      super(message, cause);
      this.incompleteCredentials = incompleteCredentials;
    }
   ...
   * Key store type.
   *
   * <p>Either a type supported by {@link KeyStore#getInstance(String)}, or a {@link SimpleKeyStore}
   * custom-reader.
   *
   ...
   * Anonymous key manager.
   *
   * <p>Never returns aliases nor credentials.
   *
   * @since 2.4
   */
   ...
   * Trust all manager.
   *
   * <p>Validate certificate chains, but does not use a trust-anchors. Use with care! This is
   * usually only used for test scenarios!
   *
   * @since 3.0
   */
  private static class X509TrustAllManager implements X509TrustManager {
     ...
     * @param client {@code true} for client's chain, {@code false}, for server's chain.
    ...
    private static void validateChain(X509Certificate[] chain, boolean client)
      ...
      if (chain != null && chain.length > 0) {
        LOGGER.debug(
            "check certificate {} for {}",
            chain[0].getSubjectX500Principal(),
            client ? "client" : "server");
        if (!CertPathUtil.canBeUsedForAuthentication(chain[0], client)) {
          LOGGER.debug(
              "check certificate {} for {} failed on key-usage!",
              chain[0].getSubjectX500Principal(),
              client ? "client" : "server");
          throw new CertificateException(
              "Key usage not proper for " + (client ? "client" : "server"));
        } else {
          LOGGER.trace(
              "check certificate {} for {} succeeded on key-usage!",
              chain[0].getSubjectX500Principal(),
              client ? "client" : "server");
        }
        ...
        try {
          ...
          LOGGER.trace(
              "check certificate {} [chain.length={}] for {} validated!",
              chain[0].getSubjectX500Principal(),
              chain.length,
              client ? "client" : "server");
        } catch (GeneralSecurityException e) {
          LOGGER.debug(
              "check certificate {} for {} failed on {}!",
              chain[0].getSubjectX500Principal(),
              client ? "client" : "server",
              e.getMessage());
