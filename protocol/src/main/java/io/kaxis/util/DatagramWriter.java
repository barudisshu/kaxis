...
public final class DatagramWriter {
   ...
   *
   * @param secureClose {@code true}, clear internal buffer on {@link #close()}, {@code false},
   *     don't clear internal buffer.
   */
  ...
  /**
   * Gets the var length bits.
   *
   * @param numBits number bits
   * @return integer
   */
  ...
  /**
   * Gets the null length values.
   *
   * @param varLengthBits null bits
   * @return integer
   */
    ...
    switch (varLengthBits) {
      ...
      default:
        throw new IllegalArgumentException(
            "Var length Bits must be a multiple of 8, not " + varLengthBits + "!");
    }
  }
   ...
   * @throws IllegalArgumentException if the number of bits is not in range 1..64, or the provided
   *     data contains more bits than that number.
   */
  public void writeLong(final long data, final int numBits) {

    if (numBits < 0 || numBits > 64) {
      throw new IllegalArgumentException(
          String.format("Number of bits must be 1 to 64, not %d", numBits));
    }
    if (numBits < 64 && (data >> numBits) != 0) {
      throw new IllegalArgumentException(
          String.format("Truncating value %d to %d-bit integer", data, numBits));
    }
    ...
    if ((numBits & 0x7) == 0 && !isBytePending()) {
      ...
      for (int i = numBits - 1; i >= 0; i--) {
        ...
        if (bit) {
          ...
          currentByte |= (byte) (1 << currentBitIndex);
        }
   ...
   *
   * <p>Calls {@link #ensureBufferSize(int)}.
   *
   ...
   * Write byte (internal).
   *
   * <p>Doesn't call {@link #ensureBufferSize(int)}, that must be called ahead!
   *
   ...
   * @throws IllegalArgumentException if the number of bits is not in range 1..32, or the provided
   *     data contains more bits than that number.
   */
  public void write(final int data, final int numBits) {
    if (numBits < 0 || numBits > 32) {
      throw new IllegalArgumentException(
          String.format("Number of bits must be 1 to 32, not %d", numBits));
    }
   ...
   *
   * <p>Write first the length of the variable bytes according the size in {@code numBits}. For
   * {@code null}, {@code -1} is written as length.
   *
  ...
  public void writeVarBytes(byte[] bytes, int numBits) {
    ...
    if (bytes == null) {
      ...
      if (nullLengthValue == bytes.length) {
        throw new IllegalArgumentException(
            bytes.length + " bytes is too large for " + numBits + "!");
      }
      if (numBits < varLengthBits && (bytes.length >> numBits) != 0) {
        throw new IllegalArgumentException(
            String.format("Truncating value %d to %d-bit integer", bytes.length, numBits));
      }
  ...
  /**
   * Write Bytes with length. Write first the length of the variable bytes according the size in
   * <code>numBits</code>. For <code>null</code>, <code>-1</code> is written as length.
   *
   * @param bytes bytes to write
   * @param numBits number of bits for encoding the length.
   */
  public void writeVarBytes(Bytes bytes, int numBits) {
    writeVarBytes(bytes == null ? null : bytes.getByteArray(), numBits);
  }
   ...
   * @throws IllegalArgumentException if number of bits doesn't align to bytes (multiple of 8).
   ...
   *
   * <p>This size includes all bytes after the size-field up to the current end. Intended for fast
   * and simple length encoding.
   *
   * <pre>
   ...
   * </pre>
   *
   * @param position byte position of size. Maybe return value of {@link #space(int)}.
  ...
  /**
   * Reset writer. If {@link #DatagramWriter(boolean)} secure close is enabled, clear the related
   * byte array before releasing it.
   *
   * @since 3.0
   */
  public void reset() {
  ...
  /**
   * Close writer, release resources. If {@link #DatagramWriter(boolean)} secure close is enabled,
   * clear the related byte array before releasing it.
   */
  public void close() {
    reset();
    buffer = Bytes.EMPTY_BYTES;
   ...
   * Set new buffer size.
   *
   * <p>Increase buffer size, copy already written data.
   *
   ...
   * Ensure buffer size.
   *
   * <p>Enlarge buffer, if additional size exceed the current allocated buffer.
   *
   * @param add additional bytes
   * @since 3.0
   */
  private void ensureBufferSize(int add) {
    int size = count + add;
   ...
   * @since 3.0
   */
  private int calculateBufferSize(int size) {
  ...
  public String toString() {
    ...
    if (byteArray.length != 0) {
      ...
      var builder = new StringBuilder(byteArray.length * 3);
