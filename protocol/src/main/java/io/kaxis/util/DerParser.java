 ...
 *
 * <p>Based on <a
 * href="https://github.com/groovenauts/jmeter_oauth_plugin/blob/master/jmeter/src/main/java/org/apache/jmeter/protocol/oauth/sampler/PrivateKeyReader.java">PrivateKeyReader.java</a>
 */
final class DerParser {
  ...
  Asn1Object readAsn1Object() throws IOException {
    ...
    // getLength() can return any 32 bits integer, so ensure that a corrupted encoding won't
    // force us into allocating a very large array
    if (length > maxAsnObjectLength) {
      throw new IOException(
          "Invalid DER: size of ASN.1 object to be parsed "
              + "appears to be larger than the size of the key file itself.");
    }
   ...
   *       "1" and bits 7-1 give the number of additional length octets. Second and following octets
   *       give the length, base 256, most significant digit first.
   * </ul>
   *
